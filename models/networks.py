...
class ErosionLayer(nn.Module):
    def __init__(self, width=512, iterations=10, output_water=False, random_param=False, blend_inputs=False, use_convs=False, set_rain=False, no_parameters=False):
        ...
        if no_parameters:
            ...
            self.MLP = nn.Sequential(nn.Linear(latent_size, 1024), nn.ReLU(True), nn.InstanceNorm1d(1024), nn.Linear(1024, 1024), nn.ReLU(True), nn.InstanceNorm1d(1024), nn.Linear(1024, 1024), nn.ReLU(True), nn.InstanceNorm1d(1024), nn.Linear(1024, 7))
            return
        elif random_param:
