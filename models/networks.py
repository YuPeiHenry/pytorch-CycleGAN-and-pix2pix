...
class UnetResBlockSplit(nn.Module):
    ...
    def forward(self, x, slope):
        ...
        for i in range(self.depth):
            intermediate = getattr(self, "b1_conv1_" + str(i))(intermediate)
...
class ErosionLayer(nn.Module):
    def __init__(self, width=512, iterations=10, output_water=False, random_param=False, blend_inputs=False, use_convs=False, set_rain=False, no_parameters=False):
    ...
    def forward(self, input_terrain, original_terrain, iterations=None, store_water=False, init_water=None, set_rain=None, latent=None, alpha=1):
        ...
        if not latent is None:
            ...
            rain_rate = alpha * parameters[:, 0].view(-1, 1, 1) + (1 - alpha) * -4.186
            evaporation_rate = alpha * parameters[:, 1].view(-1, 1, 1) + (1 - alpha) * -3.89
            min_height_delta = alpha * parameters[:, 2].view(-1, 1, 1) + (1 - alpha) * -3.908
            gravity = alpha * parameters[:, 3].view(-1, 1, 1) + (1 - alpha) * 3.399
            sediment_capacity_constant = alpha * parameters[:, 4].view(-1, 1, 1) + (1 - alpha) * 3.910
        ...
        for i in range(0, iterations):
            # Add precipitation.
            if not store_water:
                water = water + self.relu(torch.exp(rain_rate) * self.random_rainfall[:, i])
            ...
            #velocity_2 = velocity ** 2 + torch.exp(gravity) * height_delta
