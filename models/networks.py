...
class ErosionLayer(nn.Module):
    def __init__(self, width=512, iterations=10, output_water=False, random_param=False):
        ...
        #inf
        if random_param:
            self.rain_rate = torch.nn.Parameter(torch.cuda.DoubleTensor([np.random.uniform(-7, -4)]))
            self.evaporation_rate = torch.nn.Parameter(torch.cuda.DoubleTensor([np.random.uniform(-10, -7)]))
            self.min_height_delta = torch.nn.Parameter(torch.cuda.DoubleTensor([np.random.uniform(-11, -9)]))
            ...
            self.deposition_rate = torch.nn.Parameter(torch.cuda.DoubleTensor([np.random.uniform(-9, -6)]))
        else:
