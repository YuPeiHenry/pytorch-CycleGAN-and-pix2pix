...
class UnetModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.netG = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, opt.maxFilters, opt.norm_G,
                                      not opt.no_dropout, opt.init_type, opt.init_gain, self.gpu_ids, downsample_mode=opt.downsample_mode, upsample_mode=opt.upsample_mode, upsample_method=opt.upsample_method, linear=opt.linear, numDownsampleConv=opt.downsampleConv, numUpsampleConv=opt.upsampleConv)
        if opt.use_erosion:
