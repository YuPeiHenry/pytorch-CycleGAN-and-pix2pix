...
class Gan128Model(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        parser.set_defaults(norm='batch', norm_G='batch', netG='unet_128', netD='n_layers', dataset_mode='twocat', preprocess='resize', load_size=512, input_nc=1, output_nc=3, upsample_mode='upsample', no_flip=True)
        parser.set_defaults(save_epoch_freq=4, display_id=0, niter=60, niter_decay=0, save_latest_freq=481, lr=0.00005)
    ...
    def __init__(self, opt):
        ...
        self.loss_names = ['G', 'G_full', 'D_128', 'D_512', 'D_full']
        # specify the images you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        self.visual_names = ['full_real_A', 'fake_B1', 'full_fake_B', 'full_real_B', 'predict_B1', 'predict_B']
        # specify the models you want to save to the disk. The training/test scripts will call <BaseModel.save_networks> and <BaseModel.load_networks>
        self.preload_names = ['U', 'G', 'Classifier', 'D_128', 'D_512', 'D_full']
        self.model_names = ['U', 'G', 'D_128', 'D_512', 'D_full']
        # define networks
    ...
    def backward_D_512(self):
        ...
        if self.isTrain:
            self.loss_D_512 = (self.criterionGAN(pred_fake - torch.mean(pred_real) + 1, False) + self.criterionGAN(pred_real - torch.mean(pred_fake) - 1, True)) / 10
    ...
    def backward_G(self):
        ...
        fake_AB = torch.cat((self.real_A, self.real_B1, self.fake_B), 1)
        pred_fake = self.netD_512(fake_AB)
        real_AB = torch.cat((self.real_A, self.real_B1, self.real_B), 1)
        pred_real = self.netD_512(real_AB)

        self.loss_G = self.loss_G / 2 + self.criterionL1(self.real_B, self.fake_B) * 10 + self.criterionGAN(pred_fake - torch.mean(pred_real) - 1, True) / 10
        self.loss_G.backward()

    def backward_G_full(self):
        interval = self.opt.load_size // 4
        self.loss_G_full = 0
        for batch_num in range(self.real_A_128.shape[0]):
            full_fake_B_single = self.full_fake_B.clone()[0, :, :, :].unsqueeze(0)
            self.optimizer_G.zero_grad()

            fake_B1_single = self.netU(self.real_A_128[batch_num, :, :, :].unsqueeze(0))#, self.noise_inputs)
            fake_B1_single_ = self.from128(fake_B1_single)
            full_real_A_single = self.full_real_A[batch_num, :, :, :].unsqueeze(0)
            for i in range(4):
                for j in range(4):
                    X = interval * i
                    Y = interval * j
                    fake_B2_single = fake_B1_single_[:, :, X:X + interval, Y:Y + interval]
                    real_A_single = full_real_A_single[:, :, X:X + interval, Y:Y + interval]
                    fake_B_single = self.netG(torch.cat((real_A_single, fake_B2_single), 1))
                    full_fake_B_single[:, :, X:X + interval, Y:Y + interval] = fake_B_single
            fake_AB = torch.cat((full_real_A_single, full_fake_B_single), 1)
            pred_fake = self.netD_full(fake_AB)
            real_AB = torch.cat((full_real_A_single, self.full_real_B[batch_num, :, :, :].unsqueeze(0)), 1)
            pred_real = self.netD_full(real_AB)
            loss_G_full = self.criterionGAN(pred_fake - torch.mean(pred_real) - 1, True)
            loss_G_full.backward()
            self.optimizer_G.step()
            self.loss_G_full = self.loss_G_full + loss_G_full.detach() / self.real_A_128.shape[0]

    def optimize_parameters(self):
        self.set_requires_grad(self.netD_128, True)
        self.set_requires_grad(self.netD_512, True)
        self.set_requires_grad(self.netD_full, True)
        self.optimize_parameters_D()
        self.set_requires_grad(self.netD_128, False)
        self.set_requires_grad(self.netD_512, False)
    ...
    def optimize_parameters_G(self):
        ...
        self.fake_B1 = self.netU(self.real_A_128, self.noise_inputs)
        fake_AB = torch.cat((self.real_A_128, self.fake_B1), 1)
        pred_fake = self.netD_128(fake_AB)
        real_AB = torch.cat((self.real_A_128, self.real_B1), 1)
        pred_real = self.netD_128(real_AB)
        self.loss_G = self.criterionL1(self.to128(self.full_real_B), self.fake_B1) * 100 + self.criterionGAN(pred_fake - torch.mean(pred_real) - 1, True)
        self.loss_G.backward()
        ...
        self.loss_G = self.cumulative_loss_G
        self.backward_G_full()

    def run_optimizers_G(self):
        self.optimizer_G.zero_grad()
        self.fake_B = self.netG(torch.cat((self.real_A, self.fake_B2), 1))
        self.backward_G()
        self.optimizer_G.step()

    def run_optimizers_D(self):
