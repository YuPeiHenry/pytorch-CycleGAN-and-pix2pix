...
class ExrDataset(BaseDataset):
    def __init__(self, opt):
        ...
        for A1_path in self.A1_paths:
            A1_img = exrlib.read_exr_float32(A1_path, list(self.input_names[self.input_channels]))[0].transpose(2, 0, 1).reshape(len(self.input_channels), -1)
        ...
        for B_path in self.B_paths:
            B_img = exrlib.read_exr_float32(B_path, list(self.output_names[self.output_channels]))[0].transpose(2, 0, 1).reshape(len(self.output_channels), -1)
    ...
    def __getitem__(self, index):
        ...
        A1_img = exrlib.read_exr_float32(A1_path, list(self.input_names[self.input_channels]))[0]
        B_img = exrlib.read_exr_float32(B_path, list(self.output_names[self.output_channels]))[0]
